...
var tlSnot = new TimelineMax({repeat: -1, repeatDelay: 10 + Math.random(10)});
var tlCat = new TimelineMax({repeat: -1, repeatDelay: 10 + Math.random(10)});
...
var tlMustash = new TimelineMax({repeat: -1, repeatDelay: 10 + Math.random(10)});
...
var tlTails = new TimelineMax({repeat: -1, repeatDelay: 10 + Math.random(10)});
...
// Random
function snotDrip() {
	tlSnot.to(snot, 0.8, {ease: Elastic.easeOut.config(2, 0.5), repeat: 1, repeatDelay: 3, yoyo: true, transformOrigin: "125% 0%", scale: 3, scaleY: 6, opacity: 1, x:3, y: 3}, 2)
};
...
function shoeTime() {
	...
	function resetTimer() {
	   	clearTimeout(t);
	   	t = setTimeout(shoeTime, 15000) // 1000 milisec = 1 sec
	}
